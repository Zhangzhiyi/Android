...
public class ActivityMain extends ListActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (intent.getData() == null) {
			...
			if(intent.getAction().equals("android.intent.action.VIEW")){
				...
				{
						c.moveToFirst();
						Log.i("id", String.valueOf(c.getLong(0)));
						Log.i("title", c.getString(1));
						Log.i("content", c.getString(2));
						Log.i("time",c.getString(3));
				}
		...
		if (haveItems) {
			...
			if (getListView().getSelectedItemId() > 0) {
				...
				Uri uri = ContentUris.withAppendedId(getIntent().getData(),
						getSelectedItemId());//构造DiaryColumns.CONTENT_URI+Id这个心的Uri
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case MENU_ITEM_EDIT:
			...
			intent.setData(item.getIntent().getData());//注意这里是Item.getIntent().getData

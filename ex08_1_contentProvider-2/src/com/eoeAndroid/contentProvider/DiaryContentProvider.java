...
public class DiaryContentProvider extends ContentProvider {
	...
	static {
		...
		sUriMatcher.addURI(Diary.AUTHORITY, SearchManager.SUGGEST_URI_PATH_QUERY, SEARCH_SUGGEST);
		sUriMatcher.addURI(Diary.AUTHORITY, SearchManager.SUGGEST_URI_PATH_QUERY+"/*", SEARCH_SUGGEST);
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		case SEARCH_SUGGEST:
			qb.setTables(DIARY_TABLE_NAME);
			//这句返回始终都是字符串SearchManager.SUGGEST_URI_PATH_QUERY
			//String query = uri.getLastPathSegment().toLowerCase(); 
			String query = selectionArgs[0];//这句才是返回搜索输入框输入的字符串
			Log.i("query", query);
			/** 替换列名(注意替换写法)，因为系统searche只能识别 _ID,SearchManager.SUGGEST_COLUMN_TEXT_1...具体看SDK**/
			
			HashMap<String,String> map = new HashMap<String,String>();
			map.put(DiaryColumns._ID, DiaryColumns._ID);
			/**将系统_ID用SearchManager.SUGGEST_COLUMN_INTENT_DATA_ID替换就可以查到点击某个suggestion的id**/
			map.put(SearchManager.SUGGEST_COLUMN_INTENT_DATA_ID, DiaryColumns._ID+" AS "+SearchManager.SUGGEST_COLUMN_INTENT_DATA_ID);
			map.put(SearchManager.SUGGEST_COLUMN_TEXT_1,DiaryColumns.TITLE+" AS "+SearchManager.SUGGEST_COLUMN_TEXT_1);
			map.put(SearchManager.SUGGEST_COLUMN_TEXT_2,DiaryColumns.BODY+" AS "+SearchManager.SUGGEST_COLUMN_TEXT_2);				
			qb.setProjectionMap(map);
			String[] Columns = {DiaryColumns._ID,SearchManager.SUGGEST_COLUMN_TEXT_1,SearchManager.SUGGEST_COLUMN_TEXT_2,SearchManager.SUGGEST_COLUMN_INTENT_DATA_ID};
			//String[] Columns = {DiaryColumns._ID,SearchManager.SUGGEST_COLUMN_TEXT_1,SearchManager.SUGGEST_COLUMN_TEXT_2};
			SQLiteDatabase db = mOpenHelper.getReadableDatabase();
			Cursor c = qb.query(db, Columns,DiaryColumns.TITLE+" like "+"'%"+query+"%'", null, null,
					null, null);
			return c;
